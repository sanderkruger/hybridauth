...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2 {
    ...
    function setUserStatus($status) {
        ...
        $response = $this->api->post('https://www.googleapis.com/plusDomains/v1/people/me/activities', array('object' => array('originalContent' => $status['message'])));
    ...
    function getFollowersCount() {
        ...
        if ($this->api->authenticated() == 'true') {
            $key = SocialNetwork::where('name', 'Google')->first();
            $response = $this->api->api('https://www.googleapis.com/plusPages/v2/people/me?key='. $key->api_key);
            ...
            throw new Exception("Could not get user's google plus followers count! {$this->providerId} returned an error. Authentication Failed");
        }
    ...
    function getUserPosts() {
        ...
        $key = SocialNetwork::where('name', 'Google')->first();
        $response = $this->api->api('https://www.googleapis.com/plusPages/v2/people/me/activities/public?key'. $key->api_key);
    ...
    function revokeAccess() {
        ...
        $response = $this->api->api("https://accounts.google.com/o/oauth2/revoke?token=".$this->token('access_token'));
        
        // check the last HTTP status code returned
        if ($this->api->http_code != 200) {
            throw new Exception("Could not revoke access from user's google plus acc! {$this->providerId} returned an error. " . $this->errorMessageByStatus($this->api->http_code));
        }
